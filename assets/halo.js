...
$( (function(){
  ...
  var scrolled ,offscreen, oldScrolled = scrolled, status = false, checkSize = true;
  var show;
  ...
  var onresize = function(evt){
    var $stickyNavWrapper = $('.sticky-wrapper');
    ...
    var height = $filterWrapper.css({ overflow: 'hidden', height: ''}).height();	$filterWrapper.css('overflow','');
 
    /* does not catch the sticky status always as sometimes the sticky lib initialized after us
    screenTop = $stickyNavWrapper.hasClass('is-sticky') ? $stickyNavWrapper.height() : 0;
    */
    screenTop = $stickyNavWrapper.height() || 0; 
    
    $filterWrapper.height( height );
    wrapperTop = $filterWrapper.offset().top;
  }

  var decide = function(){
    var scrolled = $document.scrollTop();
    var scrollUp = scrolled < oldScrolled;

    if( scrollUp &&  checkSize ) 
      onresize();
    offscreen =  scrolled > wrapperTop - screenTop ;
    show =  scrollUp && offscreen; 
    if( show != status ){
      if( show ){
        console.log( 'Show filter at ' + screenTop );
        $filter.css('top', screenTop ).addClass('reveal');
      }
      else{
        console.log(' Hide filter');
        $filter.removeClass('reveal').css('top', '' );
      }
    }
